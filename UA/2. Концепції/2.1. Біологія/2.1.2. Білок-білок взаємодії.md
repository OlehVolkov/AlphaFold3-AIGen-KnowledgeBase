---
cssclasses: [note]
tags: [biology, ppi, interactions, complex]
---

# Білок-білок взаємодії (PPI)

[[🏠 Головна]] > [[UA/2.0. Індекс|Концепції]] > Біологія
🇬🇧 [[EN/2. Concepts/2.1. Biology/2.1.2. Protein-Protein Interactions|English]]

> Більшість клітинних функцій виконується не окремими білками, а **макромолекулярними комплексами**. PPI — основа сигналізації, транскрипції, реплікації, імунної відповіді.

---

## Типи PPI за стабільністю

```mermaid
quadrantChart
    title PPI: стабільність vs специфічність
    x-axis "Низька специфічність" --> "Висока специфічність"
    y-axis "Транзієнтні" --> "Стабільні"
    quadrant-1 Специфічні комплекси
    quadrant-2 Стабільні неспецифічні
    quadrant-3 Слабкі взаємодії
    quadrant-4 Сигнальні взаємодії
    Рибосома: [0.4, 0.95]
    Антитіло-антиген: [0.9, 0.7]
    Кінази-субстрат: [0.8, 0.3]
    Шаперони: [0.3, 0.5]
    Рецептор-ліганд: [0.85, 0.6]
```

## Інтерфейс взаємодії

Площа інтерфейсу (BSA — Buried Surface Area):

$$\mathrm{BSA} = \frac{1}{2}\bigl[\mathrm{SASA}(A) + \mathrm{SASA}(B) - \mathrm{SASA}(AB)\bigr]$$

| Тип комплексу | BSA (Å²) | Приклад |
|---------------|----------|---------|
| Слабка взаємодія | 500–1000 | Сигнальні пептиди |
| Типовий гетеродимер | 1000–2000 | Більшість PPI |
| Великий стабільний | 2000–5000 | Антиген-антитіло |
| Постійний комплекс | >5000 | Рибосома, протеасома |

## Ключові фізичні сили на інтерфейсі

$$\Delta G_\text{bind} = \Delta G_\text{elec} + \Delta G_\text{vdW} + \Delta G_\text{hphob} + \Delta G_\text{HB} + \Delta G_\text{sol}$$

**Гарячі точки (hot spots)** — залишки, мутація яких знижує афінність $> 2$ ккал/моль. Зазвичай: Trp, Arg, Tyr.

## Класифікація за симетрією

```mermaid
flowchart TD
    PPI["Білок-білок комплекси"]:::root
    HOM["Гомомери<br/>(однакові ланцюги)"]:::hom
    HET["Гетеромери<br/>(різні ланцюги)"]:::het
    SYM["Симетричні<br/>C₂, C₃, D₂..."]:::sym
    ASYM["Асиметричні"]:::asym
    OBL["Облігатні<br/>(завжди разом)"]:::obl
    NON["Необлігатні<br/>(транзієнтні)"]:::non

    PPI --> HOM & HET
    HOM --> SYM & ASYM
    HET --> OBL & NON

    classDef root fill:#1e3a5f,stroke:#3b82f6,color:#93c5fd
    classDef hom  fill:#312e81,stroke:#7c3aed,color:#c4b5fd
    classDef het  fill:#14532d,stroke:#22c55e,color:#86efac
    classDef sym  fill:#1e3a5f,stroke:#3b82f6,color:#93c5fd
    classDef asym fill:#1e3a5f,stroke:#3b82f6,color:#93c5fd
    classDef obl  fill:#14532d,stroke:#22c55e,color:#86efac
    classDef non  fill:#7c2d12,stroke:#ea580c,color:#fdba74
```

## AlphaFold 3 і PPI

AF3 досягає **ipTM > 0.8** на складних гетеромерних комплексах. Ключові метрики:

| Метрика | Що вимірює | Порогові значення |
|---------|------------|------------------|
| **ipTM** | Точність інтерфейсу (predicted TM-score) | >0.8 ✅, 0.6–0.8 ⚠️, <0.6 ❌ |
| **pDockQ** | Впевненість у якості докінгу | >0.5 ✅ |
| **PAE on interface** | Помилка позиціонування на інтерфейсі (Å) | <5 Å ✅ |

> Berman et al. PDB. DOI: [10.1093/nar/28.1.235](https://doi.org/10.1093/nar/28.1.235)
> Evans et al. (2022). *Protein complex prediction with AlphaFold-Multimer*.
> DOI: [10.48550/arXiv.2109.22paym](https://doi.org/10.1101/2021.10.04.463034)

---

## Пов'язані нотатки

- [[UA/2. Концепції/2.1. Біологія/2.1.1. Згортання білків]]
- [[UA/2. Концепції/2.3. Структурна-Біоінформатика/2.3.3. DockQ]]
- [[UA/1. AlphaFold3/1.3. Результати/1.3.1. Точність по типах комплексів]]
