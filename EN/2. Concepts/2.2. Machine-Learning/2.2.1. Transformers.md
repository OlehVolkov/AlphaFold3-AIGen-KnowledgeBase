...
## LoRA for transformer adaptation

`LoRA` (Low-Rank Adaptation) keeps the pretrained matrix $W$ frozen and learns a low-rank update:

$$W' = W + \Delta W, \qquad \Delta W = \frac{\alpha}{r}BA$$

where $W \in \mathbb{R}^{d_{\text{in}}\times d_{\text{out}}}$, $B \in \mathbb{R}^{d_{\text{in}}\times r}$, $A \in \mathbb{R}^{r\times d_{\text{out}}}$, and $r \ll \min(d_{\text{in}}, d_{\text{out}})$.

### Why it is possible

- Fine-tuning updates for large transformers are often low intrinsic rank.
- The largest parameter blocks are usually linear projections in attention and MLP layers.
- Many downstream tasks need redirection of existing features rather than a full rewrite of the base model.

### Common insertion points

| Transformer family | Typical LoRA targets | Typical goal |
|---|---|---|
| Decoder-only LLM | `q_proj`, `k_proj`, `v_proj`, `o_proj`, MLP up/down projections | instruction or domain adaptation |
| Encoder or encoder-decoder transformer | attention projections and feed-forward layers | classification, retrieval, translation |
| Protein transformer / pLM | attention projections, sometimes output MLP | domain transfer, biological specialization |

### Why it matters

- Only a small fraction of parameters remains trainable.
- The base checkpoint stays reusable across many tasks.
- Adapters are small enough to swap per task or per domain.
- Training memory drops further when LoRA is combined with low-bit frozen weights (`QLoRA`).

### Limitations

- Very small rank can underfit large domain shifts.
- If the bottleneck is missing context length, poor tokenization, or a weak base model, LoRA cannot fix it.
- Serving many task-specific adapters adds operational complexity.

## Quantization for transformer inference and tuning

Quantization stores weights or activations in lower precision:

$$q = \mathrm{round}\!\left(\frac{w}{s}\right) + z,
\qquad
w \approx s(q-z)$$

where $s$ is a scale and $z$ is a zero-point for asymmetric schemes.

### Main approaches

| Approach | What is quantized | Typical setting | Strength | Trade-off |
|---|---|---|---|---|
| PTQ | weights after training | `INT8`, `INT4` | no retraining | quality depends on calibration |
| QAT | weights + activations during training | `INT8` or lower | best accuracy retention | highest engineering cost |
| Weight-only quantization | mostly weights | `W4A16`, `W8A16` | strong memory savings, simple serving path | activations and KV cache still expensive |
| `QLoRA` | frozen base weights + LoRA adapters | typically 4-bit base + BF16 adapters | efficient fine-tuning | training path is more specialized |

### Transformer-specific constraints

- Attention outlier channels can dominate quantization error.
- Long-context serving is often limited by `KV cache`, not only by weights.
- Weight-only `INT4` is often practical for inference, while full activation quantization is harder for stable long-context decoding.
- Methods such as `GPTQ` and `AWQ` try to preserve the most sensitive channels or reconstruction directions.

### Practical rule of thumb

- Use full fine-tuning when the base model itself must change deeply.
- Use `LoRA` when you need a lightweight task adapter.
- Use `QLoRA` when GPU memory is the bottleneck during fine-tuning.
- Use `GPTQ`/`AWQ`-style PTQ when the main goal is cheaper inference.
